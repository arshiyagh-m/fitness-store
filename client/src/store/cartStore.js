...
const useCartStore = create((set, get) => ({
  ...
  addToCart: (product, variant, qty = 1) => {
    ...
    let newCartItems;
    if (existItem) {
      newCartItems = cartItems.map((x) => x.variant.sku === existItem.variant.sku ? { ...x, qty: x.qty + qty } : x);
    } else {
  ...
  removeFromCart: (sku) => {
    ...
    let newCartItems;
    if (existItem.qty === 1) newCartItems = cartItems.filter((x) => x.variant.sku !== sku);
    else newCartItems = cartItems.map((x) => x.variant.sku === sku ? { ...x, qty: x.qty - 1 } : x);
    set({ cartItems: newCartItems });
    localStorage.setItem('cart', JSON.stringify(newCartItems));
  },
  deleteItem: (sku) => {
