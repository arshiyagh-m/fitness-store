...
import { Search, ShoppingCart, LogIn, Menu, Dumbbell, User as UserIcon, X, ArrowLeft } from 'lucide-react';
...
const Header = () => {
  ...
  // بستن منوی کشویی وقتی کاربر بیرون کلیک می‌کند
  useEffect(() => {
  ...
  // فیلتر کردن زنده کالاها بر اساس عنوان یا برند
      ...
      <div className="container mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6 w-full lg:w-2/3">
          ...
          <div ref={searchRef} className="hidden lg:block relative w-full max-w-xl">
            ...
            {/* دراپ‌داون نتایج زنده (Live Search Dropdown) */}
              ...
              <div className="absolute top-full right-0 left-0 bg-white rounded-2xl shadow-2xl border border-gray-100 mt-2 overflow-hidden z-50">
                {searchResults.length > 0 ? (
                  <div className="divide-y divide-gray-50">
                    <div className="px-4 py-2 bg-gray-50 text-xs font-bold text-gray-400">
                      نتایج پیشنهادی:
                    </div>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-sm font-bold text-gray-800 group-hover:text-primary transition-colors line-clamp-1">
                                {product.title}
                              </p>
                    ...
                    <div 
                      ...
                      className="p-3 text-center bg-gray-50 text-xs font-bold text-primary hover:underline cursor-pointer flex items-center justify-center gap-1"
                    >
                      مشاهده همه نتایج برای "{searchTerm}" <ArrowLeft size={14} />
                    </div>
                  </div>
                ) : (
                  <div className="p-6 text-center text-sm text-gray-500 font-medium">
                    مکصلی با مشخصات "{searchTerm}" پیدا نشد.
                  </div>
                )}
              </div>
            )}
        ...
        {/* دکمه‌های سمت چپ (پروفایل و سبد خرید) */}
          ...
          {user ? (
            <div className="flex items-center gap-2 border border-gray-200 rounded-xl px-4 py-2 hover:bg-gray-50 cursor-pointer group relative">
              ...
              <div className="absolute top-full left-0 mt-2 w-48 bg-white border border-gray-100 rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                <Link to="/profile" className="block w-full text-right px-4 py-3 text-sm text-gray-700 font-bold hover:bg-gray-50 border-b border-gray-100">
                  پروفایل من
                </Link>
                {user.role === 'admin' && (
                  <Link to="/admin/dashboard" className="block w-full text-right px-4 py-3 text-sm text-primary font-bold hover:bg-gray-50 border-b border-gray-100 bg-dark">
                    پنل مدیریت
                  </Link>
                )}
                <button onClick={logout} className="w-full text-right px-4 py-3 text-sm text-rose-500 font-bold hover:bg-rose-50 rounded-b-xl">
                  خروج
                </button>
              </div>
            </div>
